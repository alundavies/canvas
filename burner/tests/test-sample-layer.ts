...
async function run(){
    ...
    let burnedTileRange : TileRange | undefined = undefined;

    for( let x=0; x<entries; x++){
        for( let y=0; y<entries; y++){
            burnedTileRange = await burner.burnImageToFitXY( './images/square-300x300.png', x/entries, y/entries, 1.0/entries, 1.0/entries);
            console.log( `TileRange that burn took place at ${burnedTileRange}`);
        }
    }

    if( !isUndefined( burnedTileRange)){
